...
void RoboCtrlNode::handle_robot_move(
    ...
    try {
        ...
        float vel = static_cast<float>(request->velocity);
        float acc = static_cast<float>(request->acceleration);

        if (vel <= 0.0f) vel = 5.0f;
        if (acc <= 0.0f) acc = 5.0f;
        ...
        if (request->move_type == 0) {
            ...
            // 先获取当前TCP位姿，作为 MoveJ 的 desc_pos 参数
            DescPose desc_pos;
            memset(&desc_pos, 0, sizeof(DescPose));
            ret = robot_->GetActualTCPPose(0, &desc_pos);
            if (ret != 0) {
                response->success = false;
                response->message = "获取当前TCP位姿失败，错误码: " + std::to_string(ret);
                return;
            }
            ...
            RCLCPP_INFO(
                this->get_logger(),
                "执行MoveJ，目标关节: [%.3f, %.3f, %.3f, %.3f, %.3f, %.3f], 当前TCP: [%.3f, %.3f, %.3f, %.3f, %.3f, %.3f], vel=%.2f, acc=%.2f",
                joint_pos.jPos[0], joint_pos.jPos[1], joint_pos.jPos[2],
                joint_pos.jPos[3], joint_pos.jPos[4], joint_pos.jPos[5],
            ...
            if (request->cartesian_pose.size() < 6) {
                response->success = false;
                response->message = "笛卡尔位姿数组长度不足，需要6个参数";
                return;
            }
            ...
            RCLCPP_INFO(
                this->get_logger(),
                "执行MoveL，目标TCP: [%.3f, %.3f, %.3f, %.3f, %.3f, %.3f], vel=%.2f, acc=%.2f",
                desc_pos.tran.x, desc_pos.tran.y, desc_pos.tran.z,
                desc_pos.rpy.rx, desc_pos.rpy.ry, desc_pos.rpy.rz,
                vel, acc
            );
...
void RoboCtrlNode::handle_robot_move_cart(
    ...
    try {
        ...
        if (request->use_increment) {
            ...
            memset(&current_tcp_pose, 0, sizeof(DescPose));

            int get_ret = robot_->GetActualTCPPose(0, &current_tcp_pose);
            if (get_ret != 0) {
                response->success = false;
                response->message = "获取当前TCP位姿失败，错误码: " + std::to_string(get_ret);
                return;
            }
            ...
            desc_pos.tran.x = static_cast<float>(request->tcp_pose.x);
            desc_pos.tran.y = static_cast<float>(request->tcp_pose.y);
            desc_pos.tran.z = static_cast<float>(request->tcp_pose.z);
            desc_pos.rpy.rx = static_cast<float>(request->tcp_pose.rx);
            desc_pos.rpy.ry = static_cast<float>(request->tcp_pose.ry);
            desc_pos.rpy.rz = static_cast<float>(request->tcp_pose.rz);
        }

        if (desc_pos.tran.x < -1000.0f || desc_pos.tran.x > 1000.0f ||
            desc_pos.tran.y < -1000.0f || desc_pos.tran.y > 1000.0f ||
            desc_pos.tran.z < 0.0f || desc_pos.tran.z > 2000.0f) {
            response->success = false;
        ...
        float vel   = static_cast<float>(request->velocity);
        float acc   = static_cast<float>(request->acceleration);
        float ovl   = static_cast<float>(request->ovl);
        float blend = static_cast<float>(request->blend_time);

        if (vel <= 0.0f) vel = 5.0f;
        if (acc <= 0.0f) acc = 5.0f;
        if (ovl <= 0.0f) ovl = 100.0f;

        if (vel > 100.0f) vel = 100.0f;
        if (acc > 100.0f) acc = 100.0f;
        if (ovl > 100.0f) ovl = 100.0f;

        int tool   = request->tool < 0 ? 0 : request->tool;
        int user   = request->user < 0 ? 0 : request->user;
        int config = request->config < -1 ? -1 : request->config;

        RCLCPP_INFO(
            this->get_logger(),
            "执行MoveCart，目标TCP: [%.3f, %.3f, %.3f, %.3f, %.3f, %.3f], tool=%d, user=%d, vel=%.2f, acc=%.2f, ovl=%.2f, blend=%.2f, config=%d",
            desc_pos.tran.x, desc_pos.tran.y, desc_pos.tran.z,
            desc_pos.rpy.rx, desc_pos.rpy.ry, desc_pos.rpy.rz,
            tool, user, vel, acc, ovl, blend, config
        );
        ...
        while (motion_done == 0) {
            ...
            if (std::chrono::steady_clock::now() - start_time > timeout_duration) {
                response->success = false;
                response->message = "等待MoveCart完成超时";
                return;
            }
...
void RoboCtrlNode::handle_robot_set_speed(
    ...
    try {
        ...
        int ret = robot_->SetSpeed(speed);
        if (ret != 0) {
            response->success = false;
            response->message = "设置速度失败，错误码: " + std::to_string(ret);
            return;
        }
