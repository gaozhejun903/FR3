...
#     config_file_arg = DeclareLaunchArgument('config_file', default_value=os.path.join(pkg_dir, 'config', 'static_transforms.yaml'), description='静态TF配置文件路径')
#     static_tf_node = Node(package='tools', executable='static_tf_publisher_node', name='static_tf_publisher', parameters=[{'config_file': LaunchConfiguration('config_file')}], output='screen', emulate_tty=True)
#     detector_node = Node(package='detector', executable='detector_node_exe', name='detector_node', output='screen')
#     depth_handler_node = Node(package='depth_handler', executable='depth_processor_node', name='depth_handler_node', output='screen')
#     camera_info_interceptor_node = Node(package='camera_info_interceptor', executable='camera_info_interceptor_node', name='camera_info_interceptor_node', output='screen')
#     gripper_node = Node(package='epg50_gripper_ros', executable='epg50_gripper_node', name='gripper_node', output='screen')
#     return LaunchDescription([robot_ip_arg, robot_port_arg, robot_name_arg, state_query_interval_arg, robo_ctrl_node, high_level_node, fake_gripper_tf_publisher_node, declare_gripper_frame_cmd, declare_base_frame_cmd, declare_fake_frame_cmd, declare_reference_frame_cmd, declare_rate_cmd, config_file_arg, static_tf_node, detector_node, depth_handler_node, camera_info_interceptor_node, gripper_node])
...
def generate_launch_description():
    ...
    return LaunchDescription([
        LogInfo(msg='⚠️ robo_ctrl_L.launch.py 已废弃! 请使用: ros2 launch depth_handler depth_full.launch.py')
    ])
