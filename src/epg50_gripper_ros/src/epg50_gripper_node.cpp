...
public:
    EPG50GripperNode(): Node("epg50_gripper_node") {
        ...
        this->declare_parameter("service_prefix", "");       // 服务名前缀 (如 "R_" 用于右臂夹爪)
        ...
        std::string service_prefix = this->get_parameter("service_prefix").as_string();

        RCLCPP_INFO(
            this->get_logger(),
            "初始化EPG50夹爪, 端口: %s, 默认从站ID: 0x%02X, 服务前缀: '%s', 状态更新频率: %.1f Hz, 命令超时: %.1f s",
            port.c_str(),
            default_slave_id,
        ...
        try {
            ...
            // 创建命令服务 (支持 service_prefix 区分左右臂夹爪)
            ...
            rename_service_ = this->create_service<epg50_gripper_ros::srv::RenameGripper>(
                service_prefix + "gripper_rename",
                std::bind(&EPG50GripperNode::handle_rename, this, std::placeholders::_1, std::placeholders::_2)
            );

            // 创建状态发布器
            status_publisher_ =
                this->create_publisher<epg50_gripper_ros::msg::GripperStatus>(service_prefix + "gripper_status_stream", 10);
